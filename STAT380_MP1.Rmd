...
Gemini generated the following as one possible solution: 
...
From the above table and pie chart, we can see that Full is much more likely to appear than Partial. Full is almost ten times more frequent than partial.
...
We can see that the eliminations variable has a positive skew, or right skew, with the mean and median being below the midpoint of the range of data. This lets us know that for most matches, this player is getting aroud 11 - 18 kills, with fewer matches getting in the high range of eliminations, with only two total matches above 30.
...
Games_maps <- Games %>%
  ...
  summarize(
    ...
    median_eliminations = median(Eliminations)
  )
 
mad <- ggplot(data = Games_maps,
       mapping = aes(x = reorder(Choice, mean_deaths),
                     y = mean_deaths)) + 
  geom_segment(aes(x = Choice, xend = Choice, y = 0, yend = mean_deaths), color = "red4") +
  geom_point(color = "black") +
  scale_y_continuous(breaks = seq(0, 25, by = 1)) +
  coord_flip() +
  labs(x = "Map", y = "Mean Deaths", title = "Mean Deaths by Map")
 
mmd <- ggplot(data = Games_maps,
       mapping = aes(x = reorder(Choice, median_deaths),
                     y = median_deaths)) + 
  geom_segment(aes(x = Choice, xend = Choice, y = 0, yend = median_deaths), color = "blue4") +
  geom_point(color = "black") +
  scale_y_continuous(breaks = seq(0, 25, by = 1)) +
  coord_flip() +
  labs(x = "Map", y = "Median Deaths", title = "Median Deaths by Map")

mae <- ggplot(data = Games_maps,
       mapping = aes(x = reorder(Choice, mean_eliminations),
                     y = mean_eliminations)) + 
  geom_segment(aes(x = Choice, xend = Choice, y = 0, yend = mean_eliminations), color = "red4") +
  geom_point(color = "black") +
  scale_y_continuous(breaks = seq(0, 25, by = 1)) +
  coord_flip() +
  labs(x = "Map", y = "Mean Eliminations", title = "Mean Eliminations by Map")

mme <- ggplot(data = Games_maps,
       mapping = aes(x = reorder(Choice, median_eliminations),
                     y = median_eliminations)) + 
  geom_segment(aes(x = Choice, xend = Choice, y = 0, yend = median_eliminations), color = "blue4") +
  geom_point(color = "black") +
  scale_y_continuous(breaks = seq(0, 25, by = 1)) +
  coord_flip() +
  labs(x = "Map", y = "Median Eliminations", title = "Median Eliminations by Map")
...
From the above charts it can be observed that the map does have an effect on the number of eliminations and deaths for this player. It should also be considered that all maps have been played 15 or less times and some as low as 5 times. If the player played more games, we could gain a more confident understanding of the maps' effect on the eliminations and deaths.
...
List of Questions - Andrew Loiselle

Answer Questions - Isaac Swope

Overview and Cleanup - Andrew Loiselle
